...
import { Button, Form, FormItem, Input, Modal, Select, Space, Table, Tag, message } from 'antdv-next'
...
function onExpandedRowsChange(keys: readonly (string | number)[]) {
...
function onExport() {
  ...
  try {
    ...
    if (result.total > result.rows.length) {
      void message.warning(`只导出了前 ${result.rows.length} 条，筛选共 ${result.total} 条`)
    } else {
      void message.success(`已导出 ${result.rows.length} 条`)
    }
  } finally {
...
function onBatchDelete() {
  ...
  Modal.confirm({
    ...
    async onOk() {
      ...
      void message.success(batchDeleteDoneText(result.deleted, '个部门', result.skipped))
      selectedKeys.value = []
      await load()
    },
  })
...
function onDelete(row: SystemDept) {
  if (flat.value.some((item) => item.parentId === row.id)) {
    void message.warning('请先删除下级部门')
    return
  }
  if ((row.userCount ?? 0) > 0) {
    void message.warning('请先移走该部门下的用户')
    return
  }
  Modal.confirm({
  ...
  <AntdvPage>
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'userCount'">{{ toDept(record).userCount ?? 0 }}</template>
        ...
        <template v-else-if="column.key === 'actions'">
          <Space>
            <Button v-if="hasAnyAction('dept:create')" type="link" @click="onAppend(toDept(record))">
              下级
            </Button>
            <Button v-if="hasAnyAction('dept:update')" type="link" @click="onEdit(toDept(record))">
              编辑
            </Button>
            <Button
              v-if="hasAnyAction('dept:delete')"
              danger
              type="link"
              @click="onDelete(toDept(record))"
            >
              删除
            </Button>
          </Space>
