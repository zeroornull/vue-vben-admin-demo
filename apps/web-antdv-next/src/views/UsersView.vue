...
import { nextAntdvTableQuery } from '@/tables/antdv-sort'
...
const deptTree = computed(() => toAntdvTree(toParentOptions(catalog.value)))
...
function sortOrderOf(field: string) {
  return sort.value?.field === field ? sort.value.order : undefined
}
...
const columns = computed<TableColumnsType<SystemUser>>(() => {
  ...
  if (tableColumns.isVisible('users', 'deptId')) {
    base.push({ dataIndex: 'deptId', title: columnTitle('users', 'deptId'), width: 140 })
  }
  if (tableColumns.isVisible('users', 'roleIds')) {
    base.push({ dataIndex: 'roleIds', title: columnTitle('users', 'roleIds'), width: 180 })
  }
  ...
  if (tableColumns.isVisible('users', 'remark')) {
    base.push({ dataIndex: 'remark', title: columnTitle('users', 'remark') })
  }
...
async function onExport() {
  ...
  try {
    ...
    if (result.total > result.items.length) {
      void message.warning(`只导出了前 ${result.items.length} 条，筛选共 ${result.total} 条`)
    } else {
      void message.success(`已导出 ${result.items.length} 条`)
    }
  } catch {
    // 失败由全局错误条提示
  } finally {
...
async function onImportFile(event: Event) {
  ...
  try {
    ...
    if (failed.length) void message.warning(importCsvSummary(created, failed))
    else void message.success(importCsvSummary(created, failed))
  } catch {
...
function onTableChange(
  ...
  _filters: unknown,
  sorter: TableSorterResult<SystemUser> | TableSorterResult<SystemUser>[],
) {
  const first = Array.isArray(sorter) ? sorter[0] : sorter
  const next = nextAntdvTableQuery(
    page.value,
    pageSize.value,
    sort.value,
    paging,
...
function onBatchDelete() {
  ...
  Modal.confirm({
    ...
    async onOk() {
      ...
      void message.success(t('toast.deletedPeople', { count: result.deleted }))
...
<template>
  <AntdvPage>
    <Form layout="inline" @submit.prevent="onSearch">
      ...
      <FormItem>
        <Space>
          ...
          <Button v-access="'user:create'" type="primary" @click="onCreate">
            {{ t('action.create') }}
          </Button>
        </Space>
      </FormItem>
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'deptId'">{{ deptLabel(toUser(record).deptId) }}</template>
        <template v-else-if="column.dataIndex === 'roleIds'">{{ roleLabel(toUser(record).roleIds) }}</template>
        ...
        <template v-else-if="column.key === 'actions'">
          <Space>
            <Button
              v-if="hasAnyAction('user:update')"
              type="link"
              @click="onEdit(toUser(record))"
            >
              编辑
            </Button>
            <Button
