...
第 84 轮 Vue 3.6 Vapor 实验
```

第一期到此结束。二期路线见 [75-phase-2-roadmap.md](./75-phase-2-roadmap.md)。第 69–84 轮已落地。当前 ← 第 84 轮。二期名单到此为止；不要把默认运行时改成 3.6。执行记录见 [76](./76-round-69-catalog.md) 至 [93](./93-round-84-vue36-vapor.md)。
...
页签 + KeepAlive。工作台钉住。当时不搬拖拽和右键；第 68 轮点名补上。
...
> 已完成，执行记录见 [64-round-58-batch-session.md](./64-round-58-batch-session.md)。

用户勾选批量删。锁屏和换账号登录跨标签跟上。

## 第 59 轮 · 表格排序 + 日志日期

> 已完成，执行记录见 [65-round-59-table-sort.md](./65-round-59-table-sort.md)。

用户 / 角色 / 外链 / 操作日志记住排序。操作日志还能按日历天筛。不记页码，不记日期。

## 第 60 轮 · 批量删除补全 + 搜索最近

> 已完成，执行记录见 [66-round-60-batch-recents.md](./66-round-60-batch-recents.md)。

角色、部门、外链也能勾选批量删。有人占用就跳过。搜索记住最近去过的页。

## 第 61 轮 · 搜索键盘 + 角色 CSV

> 已完成，执行记录见 [67-round-61-search-csv.md](./67-round-61-search-csv.md)。

搜索 ↑↓ 选中再回车。角色表按筛选导出 CSV。通知已读仍不 persist。

## 第 62 轮 · 外链日志 CSV + Home/End

> 已完成，执行记录见 [68-round-62-csv-home.md](./68-round-62-csv-home.md)。

外链和操作日志按筛选导出。搜索 Home/End 跳到头尾。部门树仍不导出。

## 第 63 轮 · 部门 CSV + 搜索数字键

> 已完成，执行记录见 [69-round-63-dept-digits.md](./69-round-63-dept-digits.md)。

部门树拍扁导出，带路径。搜索空着时 1-9 跳第几条。

## 第 64 轮 · 部门导入 + 搜索 0

> 已完成，执行记录见 [70-round-64-dept-import.md](./70-round-64-dept-import.md)。

部门按导出格式导入。搜索空着时 0 跳第 10 条。

## 第 65 轮 · 角色 / 外链导入

> 已完成，执行记录见 [71-round-65-role-link-import.md](./71-round-65-role-link-import.md)。

角色和外链按导出格式导入。当时日志不导入；第 68 轮点名补了回放导入。

## 第 66 轮 · 三表列显隐

> 已完成，执行记录见 [72-round-66-table-columns.md](./72-round-66-table-columns.md)。

角色、外链、操作日志也能开关列。用户表旧偏好还能读。当时部门树不挂；第 68 轮补了列开关，仍不分页。

## 第 67 轮 · 部门展开 + iframe 重载

> 已完成，执行记录见 [73-round-67-expand-iframe.md](./73-round-67-expand-iframe.md)。

部门树记住展开。内嵌页能重新加载、能新窗口打开。当时列开关仍不挂部门树。

## 第 68 轮 · 边角一次收完

> 已完成，执行记录见 [74-round-68-leftovers.md](./74-round-68-leftovers.md)。
...
> 已完成，执行记录见 [93-round-84-vue36-vapor.md](./93-round-84-vue36-vapor.md)。

`apps/web-vapor`。Vue 锁 3.6.0-rc.4。一张无 UI 库工作区卡片。默认仍是 3.5.41。
