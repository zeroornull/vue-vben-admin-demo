...
| --- | --- | --- |
| 交互选 app 开发 | `pnpm dev` → `turbo-run` | 默认 `bun run dev`；Element：`bun run dev:ele`；Naive：`bun run dev:naive`；TDesign：`bun run dev:tdesign`；antdv-next：`bun run dev:antdv-next` |
